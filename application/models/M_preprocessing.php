...
{
    public function array_stopword()
    {
        $stopword = array(
    ...
    function insertStemming($data)
    {
        $this->db->insert('tbl_stemming', $data);
        return $this->db->insert_id();
    }

    function get_stemming()
    {
        $this->db->select("*");
    ...
    function get_stemming2($key)
    {
        $this->db->select("*");
        $this->db->from("tbl_stemming");
        $this->db->where("array LIKE '%" . $key . "%'");
        $query  = $this->db->get();
        $result = $query->result();
        return $result;
    }
